...
- `providers/fr3/` — Franka FR3 + Franka Hand. Cherry-picked subset of `frankarobotics/franka_ros2 @ jazzy` (`franka_msgs`, `franka_hardware`, `franka_semantic_components`, `franka_robot_state_broadcaster`, `franka_gripper`, `franka_fr3_moveit_config`) plus `frankarobotics/franka_description @ main` (trimmed to fr3 + franka_hand only — fer/fp3/fr3v2*/tmrv0_2/mobile/duo robots and accessories meshes dropped). [providers/fr3/UPSTREAM.md](providers/fr3/UPSTREAM.md) records SHAs and re-sync procedure (manual cherry-pick, no `git subtree pull`). Local additions: `fr3_bringup` (launch wrapper translating `simulation:=true/false` to upstream's `use_fake_hardware`, plus a sim-only `fr3_ros_controllers_sim.yaml` with position command interfaces because mock_components doesn't simulate effort dynamics), `fr3_skill_server` (proxies the 12 robot_arm_skills atoms under /fr3 + the new gripper skill), `franka_gripper_skill` (bridges `robot_skills_msgs/FrankaGripperControl` onto `franka_gripper`'s Move/Grasp/Homing actions; STOP cancels the in-flight upstream goal). libfranka comes from conda-forge (`libfranka 0.21.1`, no source vendor needed). Two minor upstream package.xml fixes pushed locally: `franka_semantic_components` adds `<depend>controller_interface/rclcpp_lifecycle/urdf</depend>`, `franka_robot_state_broadcaster` adds `<depend>visualization_msgs</depend>` — both are referenced by the CMakeLists' `THIS_PACKAGE_INCLUDE_DEPENDS` but missing from the manifest. Validated 2026-05-01: `pixi run fr3-sim-test` brings up MoveIt fake_hardware + 12 atoms + skill_server in one launch; `test_fr3_sim.xml` runs SUCCESS in 10.36s (CheckSystemReady → FrankaGripperControl HOMING → MoveToNamedConfig ready/extended/ready → MoveToJointConfig mid). For real hardware: `pixi run fr3-real-up robot_ip:=192.x.x.x` (libfranka is already in env after `pixi install`). The Franka Hand is NOT a ros2_control controller, so `check_systems` for fr3 omits `gripper_controller`; the FrankaGripperSkill itself fronts the `franka_gripper` actions in real mode and short-circuits in sim (sim never runs `franka_gripper_node` because libfranka can't connect). Critical detail: `fr3_bringup/launch/moveit.launch.py` sets `planning_scene_monitor.publish_robot_description=True` and `publish_robot_description_semantic=True` so the skill component_container can pick up the URDF + SRDF via latched topics — without these flags, MoveGroupInterface in the C++ skill atoms times out waiting 10s for the SRDF and aborts the move.
